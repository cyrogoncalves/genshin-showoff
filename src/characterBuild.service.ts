...
const scalingStats = [{
  ...
  "EM": [13, 17, 22, 27, 32],
  "ER": [3.5, 4.8, 6.1, 7.5, 8.8],
  "CR": [2.1, 2.9, 3.7, 4.5, 5.3],
  "CD": [4.2, 5.8, 7.4, 9.0, 10.5],
  "Healing": [2.4, 3.3, 4.3, 5.2, 6.1],
}, {
  ...
  "Physical DMG": [5.2, 6.7, 8.2, 9.7, 11.2, 12.7, 14.2, 15.6, 17.1],
  "Elemental DMG": [4.2, 5.4, 6.6, 7.8, 9, 10.1, 11.3, 12.5, 13.7],
  "EM": [17, 22, 26, 31, 36, 41, 45, 50, 55],
  "ER": [4.7, 6, 7.3, 8.6, 9.9, 11.3, 12.6, 13.9, 15.2],
  "CR": [2.8, 3.6, 4.4, 5.2, 6, 6.8, 7.6, 8.3, 9.1],
  "CD": [5.6, 7.2, 8.8, 10.4, 11.9, 13.5, 15.1, 16.7, 18.3],
  "Healing": [3.2, 4.1, 5.1, 6, 6.9, 7.8, 8.7, 9.6, 10.5],
}, {
  ...
  "EM": [21, 27, 33, 39, 45, 51, 57, 63, 69, 75, 80, 86, 92],
  "ER": [5.8, 7.5, 9.1, 10.8, 12.4, 14.1, 15.7, 17.4, 19.0, 20.7, 22.3, 24.0, 25.6],
  "CR": [3.5, 4.5, 5.5, 6.5, 7.5, 8.4, 9.4, 10.4, 11.4, 12.4, 13.4, 14.4, 15.4],
  "CD": [7.0, 9.0, 11.0, 12.9, 14.9, 16.9, 18.9, 20.9, 22.8, 24.8, 26.8, 28.8, 30.8],
  "Healing": [4.0, 5.2, 6.3, 7.5, 8.6, 9.8, 10.9, 12.0, 13.2, 14.3, 15.5, 16.6, 17.8],
}, {
  ...
  "EM": [25, 32, 39, 47, 54, 61, 68, 75, 82, 89, 97, 104, 111, 118, 125, 132, 139],
  "ER": [7.0, 9.0, 11.0, 12.9, 14.9, 16.9, 18.9, 20.9, 22.8, 24.8, 26.8, 28.8, 30.8, 32.8, 34.7, 36.7, 38.7],
  "CR": [4.2, 5.4, 6.6, 7.8, 9.0, 10.1, 11.3, 12.5, 13.7, 14.9, 16.1, 17.3, 18.5, 19.7, 20.8, 22.0, 23.2],
  "CD": [8.4, 10.8, 13.1, 15.5, 17.9, 20.3, 22.7, 25.0, 27.4, 29.8, 32.2, 34.5, 36.9, 39.3, 41.7, 44.1, 46.4],
  "Healing": [4.8, 6.2, 7.6, 9.0, 10.3, 11.7, 13.1, 14.4, 15.8, 17.2, 18.6, 19.9, 21.3, 22.7, 24.0, 25.4, 26.8],
}, {
...
const calculateStats = (build: CharacterBuild): BuildStats => {
  const baseStats = build.character.stats[build.ascension][build.level];
  const stats = ["DEF", ...Object.keys(scalingStats[4])].reduce((stats: BuildStats, stat: StatType) => {
    const sum = build.artifacts.reduce((artSum, art) => {
      const scalingStat = stat.replace(/Pyro|Hydro|Electro|Anemo|Cryo|Geo/, 'Elemental');
      const main = art.mainStat === stat ? scalingStats[art.rarity - 1][scalingStat][art.level] : 0;
      const sub = art.subStats[stat] || 0;
      return artSum + main + sub;
    }, 0);
    if (sum > 0) {
      stats[stat] = (stats[stat] || 0) + sum;
    }
    return stats;
  }, { ...baseStats });
